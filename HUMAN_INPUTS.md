...
`foundercalc.dev` est enregistré chez Cloudflare et sert le site ; `base_url`
pointe dessus dans `content/config.yaml`.
...
Chaque merge sur `main` est publié par `.github/workflows/deploy.yml` (secrets
`CLOUDFLARE_API_TOKEN` et `CLOUDFLARE_ACCOUNT_ID` du repo) — plus rien à lancer
à la main. Pour mémoire, le chemin manuel :
...
- Le site est servi par Cloudflare Pages, aucun serveur ni process backend
- Chaque merge sur `main` passe la CI puis est publié (`.github/workflows/deploy.yml`)
- TLS géré par Cloudflare, renouvellement compris
- Trafic : Cloudflare Web Analytics, sans cookie
...
| URL affiliée trackée | Remplacement de l'URL nue dans `content/affiliates.yaml`, `affiliate: true`, merge (publié automatiquement) |
